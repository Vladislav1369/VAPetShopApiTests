...
class TestPet:
    ...
    @allure.title("Отправка запроса на получение информации о несуществующем питомце")
    ...
    @allure.title("Добавление питомца с полным набором данных")
    def test_add_pet_full_data_set(self):
        ...
        with allure.step("Отправка запроса на создание питомца с полным набором данных"):
            response = requests.post(f"{BASE_URL}/pet", json=payload)
            response_json = response.json()
    ...
    def test_delete_pet(self, create_pet):
        ...
        with allure.step("Отправка запроса для полученния подтверждения удаления питомца"):
            response = requests.get(f"{BASE_URL}/pet/{pet_id}")
        with allure.step("Проверка статуса ответа и данных питомца"):
            assert response.status_code == 404

    @allure.title("Проверка получения списка питомцев по валидному статусу")
    ...
    def test_get_pets_by_valid_status_(self, status, expected_status_code):
    ...
    @allure.title("Проверка получения списка питомцев по невалидному статусу")
    @pytest.mark.parametrize(
        "status, expected_status_code",
        [
            ("", 400),
    ...
    def test_get_pets_by_invalid_status_(self, status, expected_status_code):
        with allure.step(f"Отправка запроса на получение питомцев по статусу {status}"):
            response = requests.get(f"{BASE_URL}/pet/findByStatus", params={"status": status})
